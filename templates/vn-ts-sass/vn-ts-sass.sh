...
progress_bar
for folder in "${src_folders[@]}" "${root_folders[@]}" "${dist_folders[@]}" "${sass_folders[@]}"; do
    mkdir -p "$folder"
done
...
progress_bar
pnpm install -D typescript@latest eslint@^8.57.0.0 @typescript-eslint/eslint-plugin@7.8.0 eslint-config-love@latest eslint-plugin-import@latest eslint-plugin-n@16.0.0 eslint-plugin-promise@^6.0.0 >/dev/null 2>&1
progress_bar
